#include <brotherBox/linked_list.h>
#include <brotherBox/key_info.h>
#include <math.h>
#include <string.h>

char* vig_encrypt(char* plaintext, char* key){

	char p, c;
	char *target;
	size_t i, p_len, k_len;

	p_len = strlen(plaintext);
	k_len = strlen(key);
	target = calloc(sizeof(char), p_len);

	for(i = 0; i < p_len; i++){
		p = plaintext[i] - 0x41;
		c = key[ i % k_len ] - 0x41;

		target[i] = ((p + c) % 26) + 0x41;
	}
	return target;
}
...
char* vig_decrypt(char* ciphertext, char* final_key){
	...
	char c, k;
	char *decrypted;
	size_t c_len, k_len, i;

	c_len = strlen(ciphertext);
	k_len = strlen(final_key);

	decrypted = calloc(sizeof(char), c_len);
	
	for(i = 0; i < c_len; i++){
		c = ciphertext[i] - 0x41;
		k = final_key[i % k_len] - 0x41;

		decrypted[i] = ((c + k) % 26) + 0x41;
	}
	return decrypted;
}

char* vig_reverse_key(char* key){
	...
	len = strlen(key);
	reversed_key = calloc(sizeof(char), len);

	for(i = 0; i < len; i++){
		k = key[i] - 0x41;
		reversed_key[i] = (26 - k) + 0x41;
	}
	return reversed_key;
}
